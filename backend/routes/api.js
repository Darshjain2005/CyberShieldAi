  ...
  try {
    ...
    if (mimetype.startsWith('image/')) {
        ...
        const groqResponse = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
          ...
          response_format: { type: 'json_object' },
          messages: [
             { role: 'system', content: 'You are a video forensics analyst. Detect if a video was synthetically generated by an AI tool. Return {"isPhishing": true, "confidence": 92, "explanation": "AI tool detected: [name the tool]"} ONLY if the filename or metadata contains EXPLICIT markers from known AI generation tools such as RunwayML, Sora, HeyGen, Synthesia, DeepFaceLab, FaceSwap, Stable Video Diffusion, or similar AI synthesis software. For all authentic videos recorded by cameras, phones, screen recorders, or edited with tools like ffmpeg, HandBrake, OBS, iMovie, Premiere Pro, or DaVinci Resolve (which produce standard codec metadata: avc1, mp4a, isom, hevc, mp42, etc.), you MUST return {"isPhishing": false, "confidence": 95, "explanation": "Authentic video. Standard codec metadata detected. No AI generation markers found."}. ABSOLUTELY CRITICAL: Missing, minimal, or plain codec metadata is completely NORMAL for all real authentic videos — most real videos have no special metadata at all. NEVER flag the absence of metadata as suspicious. Only return isPhishing:true when you see explicitly named AI video synthesis tool signatures. Respond ONLY with valid JSON.' },
